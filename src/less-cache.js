...
module.exports = LessCache = class LessCache {
  ...
  constructor(params) {
    ...
    if (this.fallbackDir) {
      this.importsFallbackDir = join(
        this.fallbackDir,
        basename(this.importsCacheDir)
      )
    }
  ...
  cacheDirectoryForImports(importPaths) {
    ...
    return join(
      this.cacheDir,
      LessCache.digestForContent(importPaths.join('\n'))
    )
  }
  ...
  getImportedFiles(importPaths) {
    let importedFiles = []
    for (var absoluteImportPath of Array.from(importPaths)) {
      var importPath = null
      ...
      var importedFilePaths =
        this.importedFilePathsByRelativeImportPath[importPath]
      if (importedFilePaths != null) {
        ...
        try {
          walkdir(absoluteImportPath, { no_return: true }, (filePath, stat) => {
            ...
            if (this.resourcePath != null) {
              return importedFiles.push(
                this.relativize(this.resourcePath, filePath)
              )
            } else {
  ...
  getCachedCss(filePath, digest) {
    ...
    let path
    try {
      cacheEntry = this.readJson(
        this.getCachePath(this.importsCacheDir, filePath)
      )
    } catch (error1) {
      error = error1
      if (this.importsFallbackDir != null) {
        try {
          cacheEntry = this.readJson(
            this.getCachePath(this.importsFallbackDir, filePath)
          )
  ...
  readFileSync(absoluteFilePath) {
    ...
    if (this.resourcePath && fs.isAbsolute(absoluteFilePath)) {
      const relativeFilePath = this.relativize(
        this.resourcePath,
        absoluteFilePath
      )
